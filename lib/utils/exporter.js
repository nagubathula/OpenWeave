/**
 * PencilDraw - Export Utilities for Next.js (PNG, SVG, JSON)
 */

export class Exporter {
  static exportPNG(canvas, projectName = 'NextJS-PencilDraw-Export') {
  ...
  static exportSVG(state) {
    ...
    elements.forEach(e => {
      ...
      if (e.type === 'rectangle') {
        ...
        if (e.text) {
          svgContent += `  <text x="${e.x + e.width / 2}" y="${e.y + e.height / 2}" font-size="${s.fontSize || 18}" fill="#1e293b" text-anchor="middle" dominant-baseline="middle">${e.text}</text>\n`;
        }
      }
    });
