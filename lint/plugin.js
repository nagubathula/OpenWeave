    ...
    return {
      TSTypeLiteral(node) {
        ...
        if (namedType) {
          context.report({
            node,
            message: `Use '${namedType}' instead of inline type literal. Import from '@openweave/core'.`
          })
        }
      }
    }
      ...
      CallExpression(node) {
        ...
        if (arg.type === 'MemberExpression' && arg.property?.type === 'Identifier') {
          if (props.has(arg.property.name)) {
            context.report({
              node,
              message: `Use the typed copy helper instead of structuredClone for '${arg.property.name}'. Import from '@openweave/core'.`
            })
          }
        }
      }
...
function createExactCoreBarrelImportRule({ description, applies, message }) {
  return createImportSourceRule({
    description,
    applies,
    check: (source) => source === '@openweave/core' && message
  })
}

const noMcpCoreBarrelImports = createExactCoreBarrelImportRule({
  description: 'Disallow MCP imports from @openweave/core root barrel — use domain subpaths',
  applies: (file) => file.includes('/packages/mcp/src/'),
  message:
    'Use a targeted @openweave/core subpath in MCP code instead of the compatibility barrel.'
})

const noCliCoreBarrelImports = createExactCoreBarrelImportRule({
  description: 'Disallow CLI imports from @openweave/core root barrel — use domain subpaths',
  applies: (file) => file.includes('/packages/cli/src/'),
  message:
    'Use a targeted @openweave/core subpath in CLI code instead of the compatibility barrel.'
})

const noScriptCoreBarrelImports = createExactCoreBarrelImportRule({
  description: 'Disallow script imports from @openweave/core root barrel — use domain subpaths',
  applies: (file) => file.includes('/scripts/'),
  message:
    'Use a targeted @openweave/core subpath or #core/* alias in scripts instead of the compatibility barrel.'
})

const noCoreSelfPackageImports = createImportSourceRule({
  description: 'Disallow @openweave/core self-imports inside packages/core/src',
  applies: (file) => file.includes('/packages/core/src/'),
  check: (source) =>
    source.startsWith('@openweave/core') &&
    'Core internals must import local modules directly instead of importing the @openweave/core public package entrypoints.'
})
...
const noCoreFrameworkImports = createImportSourceRule({
  ...
  check: (source) =>
    ...
    `@openweave/core must stay framework-agnostic; do not import '${source}'.`
})
...
const noDirectOpenWeaveBrowserStore = {
  meta: {
    docs: {
      description: 'Disallow direct window.openWeave.store access'
    }
  },
  create(context) {
    function propertyName(property) {
    ...
    function isOpenWeaveMember(node) {
      return (
        node?.type === 'MemberExpression' &&
        propertyName(node.property) === 'openWeave' &&
    ...
    return {
      MemberExpression(node) {
        ...
        if (!isOpenWeaveMember(node.object)) return
        context.report({
          node,
          message:
            'Use window.openWeave.getStore() instead of accessing window.openWeave.store directly.'
        })
      }
    }
  }
...
const noDirectOpenWeaveWindowInternals = {
  meta: {
    docs: {
      description: 'Disallow direct access to private OpenWeave window internals'
    }
  },
  create(context) {
    function propertyName(property) {
    ...
    return {
      TSTypeAliasDeclaration(node) {
        ...
        context.report({
          node,
          message:
            'Import JsonObject from @openweave/scene-graph/primitives instead of declaring a local alias.'
        })
      }
    }
  }
...
const plugin = {
  ...
  rules: {
    ...
    'no-direct-openweave-window-internals': noDirectOpenWeaveWindowInternals,
    'no-direct-openweave-browser-store': noDirectOpenWeaveBrowserStore,
