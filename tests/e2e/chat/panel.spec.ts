...
async function injectMockTransport(page: Page) {
  await page.evaluate(() => {
    const setChatTransport = window.openWeave?.setChatTransport
...
test('"Get API key" link opens external URL via window.open', async () => {
  ...
  await page.evaluate(() => {
    window.openWeave ??= {}
    window.openWeave.test = { ...window.openWeave.test, savedOpen: window.open }
  ...
  // Restore
  await page.evaluate(() => {
    const savedOpen = window.openWeave?.test?.savedOpen
