...
async function installGoogleFontsMock(page: Page, families = ['Inter', 'OpenWeave Google Font']) {
  await page.addInitScript((googleFamilies) => {
    ...
    window.fetch = async (input, init) => {
      ...
      if (url.startsWith('https://fonts.openweave.test/')) {
      ...
      if (url.startsWith('https://fonts.googleapis.com/css2')) {
        ...
        return new Response(
          `@font-face { font-family: '${family}'; font-style: normal; font-weight: 400; src: url(https://fonts.openweave.test/${encodeURIComponent(family)}.ttf) format('truetype'); }`,
...
test('font picker selects local fonts without browser web-font access', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    Object.defineProperty(window, 'queryLocalFonts', {
      ...
      value: async () => [
        ...
        {
          family: 'OpenWeave Local Font',
          fullName: 'OpenWeave Local Font Regular',
          postscriptName: 'OpenWeaveLocalFont-Regular',
          style: 'Regular'
        }
      ]
    })
  ...
  await expect(
    page.getByTestId('font-picker-item').filter({ hasText: 'OpenWeave Local Font' })
  ).toBeVisible()
  await page.getByTestId('font-picker-item').filter({ hasText: 'OpenWeave Local Font' }).click()

  await expect(page.getByTestId('font-picker-trigger')).toContainText('OpenWeave Local Font')
  await expect
    .poll(async () =>
      page.evaluate((id) => {
        const store = window.openWeave?.getStore?.()
...
test('font picker keeps bundled fonts when local and web fonts are unavailable', async ({
  ...
  await expect(
    page.getByTestId('font-picker-item').filter({ hasText: 'OpenWeave Google Font' })
  ).toHaveCount(0)
  expect(
    await page.evaluate(
      () => (window as Window & { __googleFontsFetchCount?: number }).__googleFontsFetchCount
