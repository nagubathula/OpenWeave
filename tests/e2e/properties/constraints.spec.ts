...
test('shows constraints only for eligible frame children', async ({ page }) => {
  ...
  await page.evaluate((frameId) => window.openWeave?.getStore?.()?.select([frameId]), ids.frameId)
...
test('pin diagram and selects update constraint state', async ({ page }) => {
  ...
  await expect
    .poll(() =>
      page.evaluate(
        (id) => window.openWeave?.getStore?.()?.graph.getNode(id)?.horizontalConstraint,
        ids.firstId
      )
    )
    .toBe('MAX')
...
test('multi-selection constraint changes undo in one step', async ({ page }) => {
  const ids = await createConstrainedChildren(page)
  await page.evaluate(
    ({ firstId, secondId }) => window.openWeave?.getStore?.()?.select([firstId, secondId]),
    ids
  )
  const section = propertySection(page, 'Constraints')
  ...
  await expect
    .poll(() =>
      page.evaluate(({ firstId, secondId }) => {
        const graph = window.openWeave?.getStore?.()?.graph
        return [
...
test('resizing a frame applies child constraints', async ({ page }) => {
  ...
  await expect
    .poll(() =>
      page.evaluate((id) => window.openWeave?.getStore?.()?.graph.getNode(id)?.x, ids.secondId)
    )
    .toBeGreaterThan(150)
})
