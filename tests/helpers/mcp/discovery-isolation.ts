 ...
 * clobber the real `~/Library/Application Support/OpenWeave/mcp.json`.
 *
 * Deleting that file while the OpenWeave desktop app is running breaks every
 ...
 * platform path.
 *
 * Honored via OPENWEAVE_MCP_DISCOVERY_PATH in getDiscoveryPath()
 ...
 * is respected.
 */
const tmpDiscoveryDir = join(tmpdir(), `openweave-mcp-test-${process.pid}-${randomUUID()}`)
let ownsDiscoveryPath = false
if (!process.env.OPENWEAVE_MCP_DISCOVERY_PATH) {
  process.env.OPENWEAVE_MCP_DISCOVERY_PATH = join(tmpDiscoveryDir, 'mcp.json')
 ...
 * our own `openweave-mcp-test-<pid>` dirs (with or without a UUID suffix)
...
if (ownsDiscoveryPath) {
  // Matches both the old format (openweave-mcp-test-<pid>) and the new
  // UUID-suffixed format (openweave-mcp-test-<pid>-<uuid>) so leftovers
  // from either naming convention are swept. The PID is always capture group 1.
  const stalePattern = /^openweave-mcp-test-(\d+)(?:-[a-f0-9-]+)?$/
  let entries: string[] = []
  try {
