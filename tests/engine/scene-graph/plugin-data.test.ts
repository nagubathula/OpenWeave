...
describe('plugin data deduplication', () => {
  test('deduplicateNodeChangePluginData removes duplicate pluginData entries', () => {
    const entries = [
      { pluginID: 'openweave', key: 'textDirection', value: 'RTL' },
      { pluginID: 'openweave', key: 'textDirection', value: 'RTL' },
      { pluginID: 'openweave', key: 'textDirection', value: 'RTL' }
    ]
  ...
  test('importNodeChanges with unique pluginData entries preserves all', () => {
    const entries = [
      { pluginID: 'openweave', key: 'textDirection', value: 'RTL' },
      { pluginID: 'openweave', key: 'layoutDirection', value: 'LTR' },
  ...
  test('importNodeChanges with single pluginData entry preserves it (zero-copy path)', () => {
    const entries = [{ pluginID: 'openweave', key: 'textDirection', value: 'LTR' }]
...
describe('plugin data namespace collision guards', () => {
  test('getPluginDataKeys excludes encoded shared keys but allows private slash keys', () => {
    ...
    frame.setSharedPluginData('openweave', 'bar', 'shared-value')

    const keys = frame.getPluginDataKeys()
    expect(keys).toContain('foo')
    expect(keys).toContain('foo/bar')
    expect(keys).not.toContain('openweave/bar')
    expect(keys).not.toContain('bar')
  })

  test('getSharedPluginData does not read private openweave plugin data', () => {
    const graph = new SceneGraph()
    const api = new FigmaAPI(graph)
    const frame = api.createFrame()
    ...
    // getSharedPluginData with the 'openweave' namespace should NOT
    // return the private entry (which uses {pluginId: 'openweave', key: 'textDirection'})
    const sharedValue = frame.getSharedPluginData('openweave', 'textDirection')
  ...
  test('setSharedPluginData does not delete private openweave plugin data', () => {
    const graph = new SceneGraph()
    const api = new FigmaAPI(graph)
    const frame = api.createFrame()
    ...
    expect(keys).not.toContain('openweave/custom-key')
    expect(keys).not.toContain('custom-key')
  })

  test('getSharedPluginDataKeys does not leak private openweave keys', () => {
    const graph = new SceneGraph()
    const api = new FigmaAPI(graph)
    const frame = api.createFrame()
    ...
    const sharedKeys = frame.getSharedPluginDataKeys('openweave')
    expect(sharedKeys).not.toContain('textDirection')

    // Non-openweave shared data is accessible
