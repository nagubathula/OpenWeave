...
import { buildOpenWeaveClipboardHTML } from '@openweave/core/clipboard'
...
function copiedRectangleHtml(name = 'Pasted') {
  ...
  return buildOpenWeaveClipboardHTML([pasted], source.graph)
}
...
function copiedFrameHtml() {
  ...
  return buildOpenWeaveClipboardHTML([frame], source.graph)
}
...
describe('paste to replace', () => {
  test('replaces selected nodes with pasted OpenWeave nodes', async () => {
