...
import type { SceneNode } from '@openweave/core'
...
describe('clipboard roundtrip with images', () => {
  ...
  test('round-trips image bytes through clipboard', () => {
    ...
    const html = buildOpenWeaveClipboardHTML([node], graph)
    const parsed = parseOpenWeaveClipboard(html)

    const clipboard = expectDefined(parsed, 'OpenWeave clipboard')
  ...
  test('preserves imageHash on the fill', () => {
    ...
    const fill = expectDefined(parsed, 'OpenWeave clipboard').nodes[0]?.fills[0]
  ...
  test('multiple image hashes in different nodes are all included', () => {
    ...
    const html = buildOpenWeaveClipboardHTML([node1, node2], graph)
    const parsed = parseOpenWeaveClipboard(html)
  ...
  test('nodes without image fills produce empty images map', () => {
    ...
    expect(expectDefined(parsed, 'OpenWeave clipboard').images.size).toBe(0)
  })
  ...
  test('child node image hashes are collected', () => {
    ...
    const html = buildOpenWeaveClipboardHTML([frame], graph)
    const parsed = parseOpenWeaveClipboard(html)
