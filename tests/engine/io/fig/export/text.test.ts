...
describe('text node export', () => {
  ...
  test('keeps OpenWeave text language hints out of the .fig schema', async () => {
    await initCodec()
  ...
  test('derivedTextData fields present in raw binary', async () => {
    ...
    const { decodeBinarySchema, compileSchema, ByteBuffer } =
      await import('@openweave/kiwi/schema-runtime')
    const { parseFigKiwiChunks } = await import('@openweave/core')
