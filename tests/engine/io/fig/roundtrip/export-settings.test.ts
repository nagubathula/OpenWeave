...
import { effectiveFigmaRawNodeFields, parseFigBuffer } from '@openweave/fig'
import { MAX_EXPORT_SCALE } from '@openweave/scene-graph'
  ...
  test('does not serialize export settings plugin data for a default node', async () => {
    ...
    expect(
      exportedRect?.pluginData?.some(
        (entry) => entry.pluginID === 'openweave' && entry.key === 'exportSettings'
      ) ?? false
    ).toBe(false)
  })
