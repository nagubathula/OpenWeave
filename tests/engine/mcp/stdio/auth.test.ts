...
const TEST_DIR = join(tmpdir(), `openweave-test-stdio-auth-${process.pid}`)
...
describe.skipIf(!isUnix)('MCP stdio authentication', () => {
  ...
  const origSocketEnv = process.env.OPENWEAVE_MCP_SOCKET
  const origDiscoveryEnv = process.env.OPENWEAVE_MCP_DISCOVERY_PATH

  beforeAll(() => {
    process.env.OPENWEAVE_MCP_DISCOVERY_PATH = TEST_DISCOVERY_PATH
  })
  ...
  afterAll(() => {
    if (origDiscoveryEnv === undefined) {
      ...
      process.env.OPENWEAVE_MCP_DISCOVERY_PATH = origDiscoveryEnv
    }
  })
  ...
  afterEach(async () => {
    ...
    if (origSocketEnv === undefined) {
      ...
      process.env.OPENWEAVE_MCP_SOCKET = origSocketEnv
    }
    try {
  ...
  test('auto-discovers auth token from discovery file', async () => {
    ...
    process.env.OPENWEAVE_MCP_SOCKET = TEST_SOCKET
  ...
  test('401 with explicit auth token surfaces error immediately (no retry)', async () => {
    ...
    await expect(bridge.sendRpc({ command: 'test' })).rejects.toThrow(
      'Unauthorized: check OPENWEAVE_MCP_AUTH_TOKEN'
    )
  }, 10_000)
