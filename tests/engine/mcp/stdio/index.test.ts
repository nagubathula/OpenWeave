...
const SOCKET_DIR = join(tmpdir(), `openweave-test-stdio-${process.pid}`)
...
async function createStdioClient(socketPath: string, authToken: string | null) {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (!key.startsWith('OPENWEAVE_MCP_') && value !== undefined) {
      env[key] = value
    }
  }
  if (process.env.OPENWEAVE_MCP_DISCOVERY_PATH) {
    env.OPENWEAVE_MCP_DISCOVERY_PATH = process.env.OPENWEAVE_MCP_DISCOVERY_PATH
  }
  env.PATH = process.env.PATH ?? ''
  // Only set OPENWEAVE_MCP_SOCKET when a socket path exists.
  ...
  if (socketPath) {
    env.OPENWEAVE_MCP_SOCKET = socketPath
  }
  if (authToken) {
    env.OPENWEAVE_MCP_AUTH_TOKEN = authToken
  }
  ...
  const bridgeConnected = new Promise<void>((resolve, reject) => {
    ...
    if (stderr && 'on' in stderr) {
      ...
      stream.on('data', (chunk: Buffer) => {
        ...
        if (stderrBuffer.includes('Connected to OpenWeave')) {
          clearTimeout(timer)
          resolve()
        }
      })
...
describe('MCP stdio readiness without an open document', () => {
  test('readiness marker means server reachable, while document tools report no document', async () => {
    ...
    try {
      ...
      expect(message).toContain('OpenWeave app is not connected')
