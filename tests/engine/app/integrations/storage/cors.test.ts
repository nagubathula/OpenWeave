...
describe('cloud S3 CORS helpers', () => {
  test('builds XML with required methods and wildcard headers', () => {
    const xml = buildCorsConfigurationXml(['https://app.openweave.dev', 'http://localhost:1420'])
    expect(xml).toContain('<AllowedOrigin>https://app.openweave.dev</AllowedOrigin>')
  ...
  test('builds AWS console JSON', () => {
    const json = JSON.parse(buildCorsConfigurationJson(['https://app.openweave.dev'])) as Array<{
      AllowedOrigins: string[]
      AllowedMethods: string[]
      AllowedHeaders: string[]
    }>
    expect(json).toHaveLength(1)
    expect(json[0]?.AllowedOrigins).toContain('https://app.openweave.dev')
  ...
  test('collects static web and localhost origins', () => {
    ...
    expect(origins).toContain('https://*.openweave.dev')
    expect(origins).toContain('https://*.openweave-app.pages.dev')
