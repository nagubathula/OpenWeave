...
import { parseFigFile } from '@openweave/core/io'
...
async function createFixture() {
  const dir = await mkdtemp(join(tmpdir(), 'openweave-import-cli-'))
...
test('import CLI reads embedded HTML styles without a sidecar CSS file', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'openweave-import-cli-embedded-'))
...
test('import CLI compiles Tailwind candidates before import', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'openweave-import-cli-tailwind-'))
