...
import { runOpenWeaveCLI } from '#tests/helpers/cli'
...
async function createFigFixture() {
  const dir = await mkdtemp(join(tmpdir(), 'openweave-export-cli-'))
...
test('FIG export preserves the whole document by default', async () => {
  ...
  const { stdout, stderr, exitCode } = await runOpenWeaveCLI([
    'export',
    figPath,
    '--format',
    'fig',
...
test('FIG export requires an explicit page for a partial archive', async () => {
  ...
  const { stderr, exitCode } = await runOpenWeaveCLI([
    'export',
    figPath,
    '--format',
    'fig',
...
test('export CLI writes HTML with inline styles by default', async () => {
  ...
  expect(html).toContain('data-openweave-node-id')
...
test('export CLI can write external standalone HTML assets', async () => {
  ...
  const html = await Bun.file(output).text()
  const cssPath = join(dir, 'card-external.assets', 'openweave.css')
  const css = await Bun.file(cssPath).text()
  expect(html).toContain('<link rel="stylesheet" href="card-external.assets/openweave.css">')
  expect(html).not.toContain('<style>')
