...
describe('resolvePlatformCommand', () => {
  ...
  test('defaults args to an empty array', () => {
    expect(resolvePlatformCommand('openweave-mcp-http', undefined, MAC_UA)).toEqual({
      command: 'openweave-mcp-http',
      args: []
    })
  })

  test('passes through when the user agent is unavailable (headless/non-browser)', () => {
    expect(resolvePlatformCommand('openweave-mcp-http', ['--stdio'], '')).toEqual({
      command: 'openweave-mcp-http',
      args: ['--stdio']
    })
  })
