...
describe('Tauri automation file handling', () => {
  test('creates parent directory for automation new-document paths', async () => {
    ...
    await mockTauriIPC((cmd, args) => {
      calls.push({ cmd, args })
      if (cmd === 'plugin:path|dirname') return '/tmp/openweave/nested'
      return null
    })

    await ensureTauriParentDirectory('/tmp/openweave/nested/file.fig')

    expect(calls).toEqual([
      {
        cmd: 'plugin:path|dirname',
        args: { path: '/tmp/openweave/nested/file.fig' }
      },
      {
        cmd: 'plugin:fs|mkdir',
        args: { path: '/tmp/openweave/nested', options: { recursive: true } }
      }
    ])
  })
