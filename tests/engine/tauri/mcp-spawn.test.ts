...
const DISCOVERY_PATH = '/mock/home/.openweave/mcp.json'
const DISCOVERY_JSON = JSON.stringify({
  pid: 1234,
  socketPath: '/mock/home/.openweave/mcp.sock',
  httpPort: 7600,
  authRequired: true,
    ...
    await mockTauriIPC((cmd, args) => {
      ...
      if (cmd === 'plugin:shell|spawn') {
        expect(args).toMatchObject({
          program: 'openweave-mcp-http',
          args: [],
          options: {
            env: {
              PORT: '7600',
              OPENWEAVE_MCP_AUTH_TOKEN: expect.any(String),
              OPENWEAVE_MCP_CORS_ORIGIN: 'tauri://localhost',
              OPENWEAVE_MCP_TCP: '1',
              OPENWEAVE_MCP_ROOT: '/mock/home'
            }
          }
        })
