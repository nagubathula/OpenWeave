...
- Update the Homebrew install command to use the published `openweave` cask.

### Fixes

- Fix the published MCP package so global installs include the `openweave-mcp` and `openweave-mcp-http` launchers required by desktop app integrations.

## 0.13.1 — 2026-05-29

### Fixes

- Fix the npm package contents for the CLI so Bun installs include the built `openweave` binary and runtime bundle.

## 0.13.0 — 2026-05-29

### Fixes

- Fix the published CLI package so Bun global installs run the built `openweave` binary instead of raw TypeScript sources.
...
- Fix `.fig` exports so files reopened in Figma or OpenWeave keep their pages, components, instances, text wrapping, icons, avatars, and preview thumbnail intact.
...
- Route Claude Code stdio MCP requests through the live OpenWeave app connection, including immediate disconnected errors when no document is connected.
- Keep MCP disconnected guidance focused on starting OpenWeave and opening a document.
...
- Fix `.fig` round-trips for OpenWeave component sets and variable bindings, and recompute imported layouts after opening documents.
...
- Desktop file associations — double-click `.fig` or `.pen` files in Finder/Explorer to open them in OpenWeave.
...
- Add stdio transport for MCP server — `openweave-mcp` now works as a proper stdio MCP server for Claude Code, Cursor, etc. HTTP server available as `openweave-mcp-http`.
...
- Add `open_file` and `new_document` MCP tools with `OPENWEAVE_MCP_ROOT` path scoping
...
- Fix `openweave-mcp` npm package missing `dist/stdio.js` — explicitly list entry points in tsconfig
...
- Switch `@openweave/core` build from `tsgo` + `fix-esm-import-path` to `tsdown` — fixes bare directory imports that broke Node.js and Bun consumers
...
- Fix `@openweave/core` published package containing stale import paths from before the domain module restructuring — CLI and MCP installs from npm now resolve correctly
...
- Fix `@openweave/mcp` release packaging so the published npm tarball includes its built `dist/` CLI and server entrypoints deterministically
- Fix `@openweave/core` release build configuration so CI publish jobs include Node and Bun ambient types when compiling package artifacts
...
- New CLI commands: `openweave convert` for document conversion, `openweave formats` to inspect readable/writable/exportable formats, and `openweave lint` for design consistency, structure, and accessibility checks
...
- Fix headless CLI font fallback — bundled Inter font now ships with `@openweave/core` and loads without a web server
...
- XPath query command — `openweave query design.fig "//FRAME[@width < 300]"` to find nodes by type, attributes, and tree structure using XPath selectors
- CSS Grid layout mode — select a frame, click the grid icon in the auto layout toolbar to switch from flex to grid. Configure column/row tracks (fr, fixed px, auto), column and row gaps, and per-side padding. Powered by a [Yoga fork](https://github.com/openweave/yoga/tree/grid) with cherry-picked CSS Grid PRs from upstream
...
- Tailwind CSS v4 JSX export — export selections as HTML with Tailwind utility classes (`<div className="flex gap-4 p-3">`) from the Code panel, CLI (`bun openweave export --format jsx --style tailwind`), or programmatically via `sceneNodeToJSX(id, graph, 'tailwind')`. Supports layout, sizing, colors, border radius, opacity, rotation, overflow, shadows, blur, and typography. Uses v4 spacing semantics (px/4 multiplier) with automatic fallback to arbitrary values.
- Code panel format toggle — switch between OpenWeave (custom components) and Tailwind (HTML + utility classes) output
- Homebrew tap — `brew install openweave/tap/openweave` for macOS (arm64 + x64), auto-updated on each release
...
- CLI-to-app RPC bridge — all CLI commands work against the running app when no file is specified. Start the app, then run `bun openweave tree` to inspect the live document
- VitePress docs site — user guide, reference, architecture, and development docs at openweave.dev with 6 locales (en, de, fr, es, it, pl), SEO (OG tags, hreflang, JSON-LD, sitemap), and dark theme
...
- Fix broken test imports — stale `../../src/engine/` paths updated to `@openweave/core`
...
- SVG export — export selections as SVG from the export panel, context menu, CLI (`bun openweave export --format svg`), or MCP/AI tools (`export_svg`). Supports rectangles, ellipses, lines, stars, polygons, vectors, text with style runs, gradients, image fills, effects, blend modes, clip paths, and nested groups (#46)
...
- Fix `@openweave/core` dep in MCP package: `workspace:*` for local dev (pnpm resolves at publish time)
...
- MCP server (`@openweave/mcp`) — 29 tools for headless .fig editing via stdio (Claude Code, Cursor, Windsurf) or HTTP (Hono + Streamable HTTP with sessions)
- `openweave-mcp` and `openweave-mcp-http` binaries — install globally via `bun add -g @openweave/mcp`

### Build

- All packages emit JS via tsgo + fix-esm-import-path — `@openweave/core` and `@openweave/mcp` work on Node.js without Bun
- Core package exports: `bun` condition → src (dev), `import` condition → dist (npm consumers)
- `@openweave/mcp` added to CI publish workflow
...
- Figma clipboard: copy/paste between OpenWeave and Figma
...
- Runs at [app.openweave.dev](https://app.openweave.dev)
- No installation required
- File System Access API for save/open (Chrome/Edge), download fallback elsewhere

### Documentation

- [openweave.dev](https://openweave.dev) — VitePress site with user guide, reference, and development docs
