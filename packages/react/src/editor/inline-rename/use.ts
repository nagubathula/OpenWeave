...
export interface InlineRenameState<T extends string> {
  ...
  commit: (
    id: T,
    eventOrInput: React.SyntheticEvent<HTMLInputElement> | HTMLInputElement | Event
  ) => void
  cancel: () => void
  ...
  const commit = useCallback(
    (id: T, eventOrInput: React.SyntheticEvent<HTMLInputElement> | HTMLInputElement | Event) => {
      ...
      if (eventOrInput instanceof HTMLInputElement) {
        ...
        'currentTarget' in eventOrInput &&
        eventOrInput.currentTarget instanceof HTMLInputElement
      ) {
