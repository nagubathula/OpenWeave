...
export function useColorModel(options: UseColorModelOptions) {
  ...
  const okhclSliderPreview = useMemo(() => createOkHCLSliderPreviewModel(value.okhcl), [value.okhcl])
  const okhclSliderGradient = useMemo(() => createOkHCLSliderGradientModel(value.okhcl), [value.okhcl])

  const emitColor = useCallback((nextColor: Color) => {
    if (!colorsEqual(color, nextColor)) options.onUpdate?.(nextColor)
    return nextColor
  }, [color, options.onUpdate])

  const updateHex = useCallback((input: string) => {
    const parsed = parseColor(input.startsWith('#') ? input : `#${input}`)
    return emitColor({ ...parsed, a: color.a })
  }, [color.a, emitColor])

  const setFormat = useCallback((nextFormat: ColorFieldFormat) => {
    if (format === nextFormat) return
    setLocalFormat(nextFormat)
    options.onFormatChange?.(nextFormat)
  }, [format, options.onFormatChange])

  const updateFromReka = useCallback((nextColor: RekaColorValue) => {
  ...
  const updateRGBChannel = useCallback((channel: RGBChannel, channelValue: number) => {
    return emitColor(withRGBChannel(color, channel, channelValue))
  }, [color, emitColor])

  const updateHSLChannel = useCallback((channel: HSLChannel, channelValue: number) => {
    return emitColor(withHSLChannel(value, channel, channelValue))
  }, [value, emitColor])

  const updateHSBChannel = useCallback((channel: HSBChannel, channelValue: number) => {
    return emitColor(withHSBChannel(value, channel, channelValue))
  }, [value, emitColor])

  const updateOkHCLChannel = useCallback((channel: OkHCLChannel, channelValue: number) => {
