...
export function LayoutControlsRoot({ children }: LayoutControlsRootProps) {
  ...
  const actions = useMemo(() => ({
    ...
    ctx.updateProp, ctx.updateSizeLimit, ctx.setSizeLimitToCurrent, ctx.commitSizeLimit,
    ctx.addSizeLimit, ctx.removeSizeLimit, ctx.commitProp, ctx.setAxisSizing,
    ctx.updateAxisSize, ctx.commitAxisSize, ctx.setHorizontalPadding, ctx.commitHorizontalPadding,
    ctx.setVerticalPadding, ctx.commitVerticalPadding, ctx.setAlignment, ctx.setGapAuto,
    ctx.setLayoutDirection, ctx.updateGridTrack, ctx.addTrack, ctx.removeTrack, ctx.toggleIndividualPadding
  ])
  ...
  return (
    <LayoutControlsProvider value={contextValue}>
      {renderedChildren}
