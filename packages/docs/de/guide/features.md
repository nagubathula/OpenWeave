...
**Kopieren & Einfügen mit Figma** — Knoten in Figma auswählen, <kbd>⌘</kbd><kbd>C</kbd>, zu OpenWeave wechseln, <kbd>⌘</kbd><kbd>V</kbd>. Füllungen, Konturen, Auto-Layout, Text, Effekte, Eckenradien und Vektornetzwerke bleiben erhalten. Funktioniert in beide Richtungen.
...
```sh
openweave tree design.fig          # Knotenbaum
openweave find design.fig --type TEXT  # Suche
openweave export design.fig -f png     # Rendern
openweave analyze colors design.fig    # Farbanalyse
openweave analyze clusters design.fig  # Wiederholte Muster
...
```sh
openweave tree                     # Live-Dokument
openweave export -f png            # Canvas-Screenshot
```

Alle Befehle unterstützen `--json`. Installation: `npm install -g @openweave/cli`
...
**Web** — läuft unter [app.openweave.dev](https://app.openweave.dev), als PWA auf Mobilgeräten installierbar mit touch-optimierter Oberfläche.
...
Wenn eine Schriftart lokal nicht verfügbar ist, lädt OpenWeave sie automatisch von Google Fonts. Keine manuelle Installation nötig beim Öffnen von .fig-Dateien mit unbekannten Schriften.
