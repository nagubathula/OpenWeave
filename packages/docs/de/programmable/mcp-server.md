...
OpenWeave includes an MCP (Model Context Protocol) server that lets AI coding tools — Claude Code, Cursor, Windsurf, etc. — read and modify `.fig` files headlessly.
...
- File operations are limited to `OPENWEAVE_MCP_ROOT` (defaults to current working directory)
- CORS is disabled by default; set `OPENWEAVE_MCP_CORS_ORIGIN` to allow one origin
- Optional auth token: `OPENWEAVE_MCP_AUTH_TOKEN` (client sends `Authorization: Bearer <token>` or `x-mcp-token`)
