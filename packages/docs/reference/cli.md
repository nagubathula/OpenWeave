---
title: CLI Reference
description: Complete reference for all openweave commands, options, and flags.
---
...
```sh
openweave node [file] --id <id> [--json]
```
...
```sh
openweave variables [file] [options]
```
...
| `--style` | | JSX style: `openweave` (default), `tailwind` |
...
Import HTML/CSS/Tailwind into an editable OpenWeave document.

```sh
openweave import page.html [options]
```
...
```sh
openweave analyze colors [file] [options]
```
...
```sh
openweave analyze typography [file] [options]
```
...
```sh
openweave analyze spacing [file] [options]
```
...
```sh
openweave analyze clusters [file] [options]
```
