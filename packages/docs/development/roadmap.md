---
title: Roadmap
description: OpenWeave product roadmap and Figma compatibility tracking.
---

# Roadmap

OpenWeave is moving toward production-grade Figma compatibility while keeping design documents programmable, local-first, and fast on large files.
...
- Add visual regression coverage for full multi-page `.fig` documents. `tools/visual-oracles/src/export-fixtures.ts` exports current smoke fixture pages to `/tmp` for manual comparison without committing large images; `tests/fixtures/figma-oracles/visual-comparison-report.json` records the current Figma-vs-OpenWeave oracle diff findings.
...
- Deepen component/instance editing beyond the current authoring surface (variant switching, all four property types, defaults, preferred values, reordering, conflicts, arrangement, drag membership, override recording/reset/inspection are done): nested-instance property surfacing and shared libraries are the remaining gaps.
...
- Polish the official `SKILL.md` guidance for OpenWeave so agents use the full inspect → act → render/measure → compare → iterate loop instead of relying on one-shot prompting.
...
- Store shader layer configuration in OpenWeave documents and export graceful fallbacks when a target format cannot preserve the live effect.
...
This section tracks OpenWeave's current compatibility with Figma Design features. It is based on Figma's public Help Center feature areas and the current OpenWeave scene graph, Kiwi import/export, CanvasKit renderer, UI panels, CLI, and MCP tools.
...
- **↩ Round-trip only** — imported/preserved/exported for `.fig` fidelity, but not rendered or editable as a first-class OpenWeave feature.
...
2. **Round-trip fidelity** — fields that should survive read → write → Figma import but do not need OpenWeave UI/rendering yet. These need raw-preservation and invalidation tests.
3. **Product/runtime systems** — prototypes, libraries, FigJam, Slides, Dev Mode, CMS/AI, and media timelines. These stay schema-only or raw-preserved until OpenWeave has matching product concepts.
...
| Sections                                             |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Section rendering and title pills are OpenWeave-specific approximations.                                                                                                                                                                          |
| Rectangles / rounded rectangles                      |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Per-corner radii and smoothed corners render for fills, strokes, clips, masks, and effects.                                                                                                                                                        |
| Ellipses / arcs                                      |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | `arcData` renders/exports; inspector controls support starting angle, sweep angle, and hole ratio.                                                                                                                                                                                             |
| Lines                                                |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Stroke caps/joins render but are not fully exposed in UI.                                                                                                                                                                                          |
| Polygons / stars                                     |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | `pointCount` and `starInnerRadius` modeled and editable in inspector.                                                                                                                                                                                                        |
...
| Boolean operations                                   |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Figma `BOOLEAN_OPERATION` nodes import/export; inspector supports switching operation (union/subtract/intersect/exclude) and flattening.                                                                                                                                            |
| Components                                           |     ✅ |     ✅ |       ◐ |                ✅ |      ✅ | Component metadata, descriptions, links, and publish fields mostly round-trip.                                                                                                                                                                     |
| Component sets / variants                            |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Full definition authoring: all four property types, defaults, preferred values, value renaming, reordering, conflict resolution, arrangement, drag membership, and Figma-style variant switching (incl. boolean-pair toggles).                                                                                                        |
| Instances / overrides                                |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Overrides are recorded on edit, inspectable (panel/layer-tree indicators), and resettable; instance swap, detach, and typed assignments are editable. Raw symbol overrides and derived data remain preserved for fidelity.                                                                                                           |
| Slots                                                |      ↩ |      ✅ |      ✅ |                 ↩ |       — | Component slot creation (`INSTANCE_SWAP` properties) and interactive slot picker with preferred values and shared library support. |
...
| FigJam / Slides / Code / CMS / Buzz node families    |      ↩ |      — |       — |                 ↩ |       — | Current Kiwi schema recognizes many newer Figma node families (`TABLE`, `SLIDE`, `CODE_COMPONENT`, `CMS_RICH_TEXT`, `REPEATER`, `WEBPAGE`, etc.), but OpenWeave only preserves/round-trips them where safe; they are not first-class scene nodes. |
...
| Video/GIF/media fills                                |      ↩ |      — |       — |                 ↩ |       — | Kiwi schema includes media paint/export enums, but OpenWeave has no video/GIF playback or media layer support.                                                                                                                                    |
...
| Fill / stroke / effect styles                        |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Local style definitions are modeled, selectable, editable in the Styles Dialog, and publishable to shared design systems with undo-safe detach.                                                                                                                                                                                                            |
...
| Masks                                                |     ✅ |      ◐ |      ✅ |                ✅ |      ✅ | Figma schema `mask`, `maskType`, and `maskIsOutline` fields import and export; common sibling alpha/vector/luminance mask stacks render, including consecutive mask layers. UI controls support mask type and outline mask toggles.             |
| Auto layout: vertical/horizontal                     |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Yoga-backed layout.                                                                                                                                                                                                                                |
| Auto layout: wrap                                    |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | UI toggle exists.                                                                                                                                                                                                                                  |
| Auto layout: grid                                    |     ✅ |      ◐ |      ✅ |                ✅ |      ✅ | CSS-grid auto layout is supported with container grid tracks and child cell position/span inspector controls.                                                                                                                                      |
| Padding / gaps / alignment                           |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Common flex controls are exposed.                                                                                                                                                                                                                  |
| Hug / fill / fixed sizing                            |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Min/max support is partial in UI.                                                                                                                                                                                                                  |
| Ignore auto layout / absolute positioning            |     ✅ |     ✅ |       ◐ |                ✅ |      ✅ | Modeled; UI coverage is partial.                                                                                                                                                                                                           |
| Strokes included in layout                           |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Modeled and rendered; Advanced auto-layout popover controls stroke inclusion in layout geometry.                                                                                                                                                   |
| Reverse z-index / align-content                      |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Canvas stacking order (`itemReverseZIndex`) and wrap align content (`counterAxisAlignContent`) are exposed in the Advanced layout popover.                                                                                                        |
| Constraints                                          |     ✅ |      ◐ |      ✅ |                ✅ |      ✅ | Interactive 2D pin diagram and horizontal/vertical dropdowns in inspector; tools/API expose constraints.                                                                                                                                                                  |
| Layout grids / guides                                |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Layout grids (columns, rows, grid) and guides are modeled, editable, and stylable; grid styles are selectable and authorable.                                                                                                                   |
| Text styles                                          |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Local text styles are modeled, authorable, selectable, and publishable to design system libraries via the Styles Dialog.                                           |
| Rich style runs                                      |     ✅ |     ✅ |       ◐ |                ✅ |      ✅ | Import/render/export support; editing mixed runs is partial.                                                                                                                                                                                       |
| Text auto resize                                     |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Full segmented controls in typography inspector (width and height, auto height, fixed size).                                                                                                                                                                                            |
| Text truncation / max lines                          |     ✅ |     ✅ |      ✅ |                ✅ |      ✅ | Renderer supports ending truncation; inspector controls support truncation and maxLines.                                                                                                                                                                                         |
| Text case                                            |     ✅ |      ◐ |      ✅ |                ✅ |      ✅ | Model/export/JSX support; inspector exposes Original, Uppercase, Lowercase, and Title case.                                                                                                                                                                                |
| Vertical text alignment                              |     ✅ |      ◐ |      ✅ |                ✅ |      ✅ | Modeled; inspector exposes top, middle, and bottom vertical alignment.                                                                                                                                                                                                     |
| Justified text                                       |     ✅ |      ◐ |      ✅ |                ✅ |      ✅ | Modeled; inspector exposes left, center, right, and justified horizontal alignment.                                                                                                                                                                                                                    |
| Font variations / OpenType features                  |     ✅ |     ✅ |      ✅ |                ✅ |       — | Imported `fontVariations`, ligature/caps/numeric OpenType fields, and raw features are applied; inspector exposes expandable OpenType feature toggles and underline styling.                                                                     |
| Variables: collections/modes/aliases                 |     ✅ |      ◐ |      ✅ |                ✅ |      ✅ | Color/number/string/boolean model exists; multi-mode collections switcher, mode editing, and value bindings are supported in the inspector.                                                                                                         |
...
| Variables in prototypes / expressions / conditionals |     ✅ |     ✅ |       — |                 — |       — | Prototype evaluator supports expressions, variable mutations, and conditions.                                                                                                                                                                      |
| Libraries / publish / update review                  |      ↩ |     ✅ |      ✅ |                 ↩ |       — | Shared libraries dialog supports local publishing, `.json` export/import, and canvas insertion via Assets panel.                                                                                                                                    |
...
| Smart animate / easing / spring / duration           |      — |     ✅ |      ✅ |                 — |       — | `PrototypePlayer` executes smart animate layer matching across screens, spring physics curves, and bezier transitions.                                                                                                                              |
| Interactive components                               |      — |      ◐ |       ◐ |                 — |       — | "Change to" variant reactions (click and while-hovering) play in the prototype player; smart animate and `.fig` round-trip of variant reactions are not supported.                                                                                                                                                                  |
| Dev Mode inspect / measurements / annotations        |      — |      — |       — |                 — |       ◐ | OpenWeave has CLI/MCP inspection, but not Figma Dev Mode UI.                                                                                                                                                                                      |
| Code Connect / dev resources / ready-for-dev         |      — |      — |       — |                 — |       — | Not modeled.                                                                                                                                                                                                                                       |
| Comments                                             |      — |      — |       — |                 — |       — | Not modeled.                                                                                                                                                                                                                                       |
| Version history / branches                           |      — |      — |       — |                 — |       — | Not modeled.                                                                                                                                                                                                                                       |
| Real-time collaboration                              |      — |     ✅ |      ✅ |                 — |       — | OpenWeave has its own P2P collaboration, not Figma-compatible metadata.                                                                                                                                                                           |

## Raw Kiwi metadata coverage

OpenWeave deliberately preserves many Figma/Kiwi fields even when they are not rendered or editable. These live under `SceneNode.source.fig` and are applied late during `.fig` export. A schema coverage test compares the current `fig.kiwi` `NodeChange` fields against modeled codec fields, raw-preserved fields, and intentionally schema-only metadata buckets so drift stays visible.
...
| Style IDs: fill/stroke/text/effect/grid                                 |            ✅ |       ✅ |  ✅ | Preserves style linkage for Figma; local styles are manageable via Styles Dialog and publishable across libraries.                                                                                                                                                                                                                                               |
...
7. **Slot workflows and nested-instance property surfacing** — component slots (`INSTANCE_SWAP` slot picker) and Figma's "expose properties from nested instances" are implemented alongside local/shared library publishing and consumption.
8. **Prototype/media/interaction metadata** — smart animate layer matching and spring/bezier transitions are supported in the prototype runtime; next phase focuses on imported Figma interaction metadata round-trip.
