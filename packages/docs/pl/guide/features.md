...
**Kopiuj i wklej z Figmą** — zaznacz węzły w Figmie, <kbd>⌘</kbd><kbd>C</kbd>, przełącz się na OpenWeave, <kbd>⌘</kbd><kbd>V</kbd>. Wypełnienia, obrysy, auto-layout, tekst, efekty, promienie narożników i sieci wektorowe są zachowane. Działa w obie strony.
...
```sh
openweave tree design.fig          # Drzewo węzłów
openweave find design.fig --type TEXT  # Wyszukiwanie
openweave export design.fig -f png     # Renderowanie
openweave analyze colors design.fig    # Audyt kolorów
openweave analyze clusters design.fig  # Powtarzające się wzorce
...
```sh
openweave tree                     # Aktywny dokument
openweave export -f png            # Zrzut ekranu canvasu
```

Wszystkie komendy obsługują `--json`. Instalacja: `npm install -g @openweave/cli`
...
**Web** — działa na [app.openweave.dev](https://app.openweave.dev), instalowalny jako PWA na urządzeniach mobilnych z UI zoptymalizowanym pod dotyk.
...
Gdy czcionka nie jest dostępna lokalnie, OpenWeave pobiera ją automatycznie z Google Fonts. Nie trzeba ręcznie instalować czcionek przy otwieraniu plików .fig z nieznanymi fontami.
