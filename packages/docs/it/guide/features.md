...
**Copia e incolla con Figma** — seleziona i nodi in Figma, <kbd>⌘</kbd><kbd>C</kbd>, passa a OpenWeave, <kbd>⌘</kbd><kbd>V</kbd>. Riempimenti, bordi, auto-layout, testo, effetti, raggi degli angoli e reti vettoriali vengono preservati. Funziona in entrambe le direzioni.
...
```sh
openweave tree design.fig          # Albero dei nodi
openweave find design.fig --type TEXT  # Ricerca
openweave export design.fig -f png     # Render
openweave analyze colors design.fig    # Audit colori
openweave analyze clusters design.fig  # Pattern ripetuti
openweave eval design.fig -c "..."     # Figma Plugin API
```

Quando l'app desktop è in esecuzione, ometti il file per controllare l'editor live tramite RPC:

```sh
openweave tree                     # Documento live
openweave export -f png            # Screenshot del canvas
```

Tutti i comandi supportano `--json`. Installazione: `npm install -g @openweave/cli`
...
**Web** — disponibile su [app.openweave.dev](https://app.openweave.dev), installabile come PWA su mobile con interfaccia ottimizzata per il touch.
...
Quando un font non è disponibile localmente, OpenWeave lo scarica automaticamente da Google Fonts. Nessuna installazione manuale necessaria quando si aprono file .fig con font non familiari.
