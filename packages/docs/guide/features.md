...
**Copy & paste with Figma** — select nodes in Figma, <kbd>⌘</kbd><kbd>C</kbd>, switch to OpenWeave, <kbd>⌘</kbd><kbd>V</kbd>. Fills, strokes, auto-layout, text, effects, corner radii, and vector networks are preserved. Works both ways.
...
```sh
openweave tree design.fig          # Node tree
openweave find design.fig --type TEXT  # Search
openweave export design.fig -f png     # Render
openweave analyze colors design.fig    # Color audit
openweave analyze clusters design.fig  # Repeated patterns
...
```sh
openweave tree                     # Live document
openweave export -f png            # Screenshot canvas
```

All commands support `--json`. Install: `npm install -g @openweave/cli` (or `bun add -g @openweave/cli`).
...
**Web** — runs at [app.openweave.dev](https://app.openweave.dev), installable as a PWA on mobile with touch-optimized UI.
...
When a font isn't available locally, OpenWeave fetches it from Google Fonts automatically. No manual installation needed when opening .fig files with unfamiliar fonts.
