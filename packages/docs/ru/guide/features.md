...
**Копирование и вставка с Figma** — выделите узлы в Figma, <kbd>⌘</kbd><kbd>C</kbd>, переключитесь на OpenWeave, <kbd>⌘</kbd><kbd>V</kbd>. Заливки, обводки, авто-макет, текст, эффекты, радиусы скругления и векторные сети сохраняются. Работает в обоих направлениях.
...
```sh
openweave tree design.fig          # Дерево узлов
openweave find design.fig --type TEXT  # Поиск
openweave export design.fig -f png     # Рендеринг
openweave analyze colors design.fig    # Аудит цветов
openweave analyze clusters design.fig  # Повторяющиеся паттерны
...
```sh
openweave tree                     # Текущий документ
openweave export -f png            # Скриншот канваса
```

Все команды поддерживают `--json`. Установка: `npm install -g @openweave/cli`
...
**Веб** — работает на [app.openweave.dev](https://app.openweave.dev), устанавливается как PWA на мобильных устройствах с адаптированным интерфейсом.
...
Когда шрифт недоступен локально, OpenWeave автоматически загружает его из Google Fonts. Ручная установка не требуется при открытии файлов .fig с незнакомыми шрифтами.
