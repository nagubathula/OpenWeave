# OpenWeave vs Penpot: сравнение архитектуры и производительности

Зачем сравнивать? OpenWeave существует, потому что закрытые дизайн-платформы ограничивают возможности. Понимание архитектурных различий показывает, что может предложить открытая, локальная альтернатива.
...
OpenWeave **примерно в 11 раз меньше** — и в этом вся суть. Это не упрощение, а принципиально другая архитектура.

## 2. Архитектура

### OpenWeave: монолитный клиент
...
Однопроцессная архитектура OpenWeave устраняет:
...
Архитектура Penpot оптимизирована для **многопользовательских серверных развёртываний**. OpenWeave оптимизирован для **мгновенной локальной производительности**.

## 3. Конвейер рендеринга

### OpenWeave: TS → CanvasKit WASM (напрямую)
...
- **1 пересечение границы** (JS → WASM), как и у OpenWeave — но с явными накладными расходами на сериализацию: UUID разбиваются на 4×u32, трансформации на 6×f32, заливки/обводки упаковываются в бинарный формат, базовые свойства группируются в 104-байтную структуру на фигуру
...
OpenWeave перерисовывает всю область видимости каждый кадр, потому что CanvasKit, вызываемый напрямую из TS, достаточно быстр и не нуждается в тайлинге.

### Вердикт: рендеринг

| Аспект | OpenWeave | Penpot |
|--------|-------------|--------|
...
OpenWeave использует проверенную схему Figma (194 определения Kiwi) напрямую в TypeScript — без преобразований. Penpot поддерживает собственную систему типов в Clojure/ClojureScript/Rust, требующую ручной синхронизации между всеми тремя.

## 5. Движок макетов

### OpenWeave: Yoga WASM (314 строк)
...
OpenWeave делегирует проверенной библиотеке (Yoga, используется React Native на миллиардах устройств) в 314 строках. Penpot поддерживает ~3 000+ строк собственного кода макета, дублированного на двух языках.
...
OpenWeave имеет значительное преимущество — он может читать файлы Figma нативно и даже вставлять данные из буфера обмена Figma. Penpot требует ручного экспорта/импорта и не может открывать файлы `.fig`.
...
Подход OpenWeave проще и с меньшими накладными расходами. Подход Penpot больше подходит для совместной работы (изменения сериализуемы), но ценой сложности.
...
1. **Серверная совместная работа** — централизованное многопользовательское редактирование с WebSockets, учётными записями и контролем доступа (OpenWeave использует P2P через Trystero + Yjs — без сервера, но и без контроля доступа или сохранения за пределами сессии)
2. **Экспорт PDF** — headless Chromium для серверного PDF-рендеринга (OpenWeave экспортирует SVG, но пока не PDF)
3. **Система плагинов** — полный API плагинов с изолированным выполнением
4. **Дизайн-токены** — нативная поддержка дизайн-токенов
5. **Макет CSS Grid** — собственная реализация (OpenWeave ждёт Yoga Grid)
...
OpenWeave архитектурно легче — однопроцессный CanvasKit-рендерер в ~26K строках TypeScript, совместимый с Figma по дизайну. Penpot — полноценная платформа с ~299K строками на Clojure, ClojureScript, Rust и SCSS, плюс парк Docker-сервисов. Оба предлагают совместную работу в реальном времени (с разными архитектурами: P2P vs сервер). Penpot имеет экосистему плагинов и серверный экспорт PDF; OpenWeave имеет Figma-совместимый headless-скриптинг, **90 ИИ/MCP инструментов**, экспорт SVG и нативное десктоп-приложение.
