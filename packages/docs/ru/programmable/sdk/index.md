---
title: Vue SDK
description: Создавайте редакторы на базе OpenWeave с помощью headless-компосаблов и примитивов для Vue.
---

# Vue SDK

`@openweave/react` существует для того, чтобы OpenWeave мог быть чем-то большим, чем самостоятельное дизайн-приложение.

Цель — сделать OpenWeave набором инструментов, который можно встроить в другие продукты, внутренние инструменты и редакторы под конкретные задачи, а не только использовать в виде единого стандартного интерфейса.

Приложение OpenWeave — одна из возможных компоновок этого набора. SDK — это то, с помощью чего вы создаёте другие.
...
- **Интеграция с фреймворком**: Vue-интеграция поверх `@openweave/core`
...
- [Компосаблы](/programmable/sdk/api/hooks/)
