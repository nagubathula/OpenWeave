...
```sh
openweave query design.fig "//TEXT"                    # Все текстовые узлы
openweave query design.fig "//COMPONENT"               # Все компоненты
openweave query design.fig "//INSTANCE"                # Все экземпляры
```

**По атрибутам:**

```sh
openweave query design.fig "//FRAME[@width < 300]"                # Фреймы шириной менее 300px
openweave query design.fig "//*[@cornerRadius > 0]"               # Скруглённые углы
openweave query design.fig "//*[@visible = false]"                # Скрытые узлы
openweave query design.fig "//TEXT[@fontSize >= 24]"              # Крупный текст
openweave query design.fig "//*[@opacity < 1]"                    # Полупрозрачные узлы
```

**По имени и текстовому содержимому:**

```sh
openweave query design.fig "//TEXT[contains(@name, 'Button')]"    # Имя содержит 'Button'
openweave query design.fig "//TEXT[contains(@text, 'Hello')]"     # Текст содержит 'Hello'
```

**По иерархии:**

```sh
openweave query design.fig "//SECTION//TEXT"            # Текст внутри секций
openweave query design.fig "//FRAME/TEXT"               # Прямые дочерние тексты фреймов
openweave query design.fig "//COMPONENT_SET//INSTANCE"  # Экземпляры внутри наборов компонентов
```
...
```sh
openweave tree              # просмотр текущего документа
openweave eval -c "..."     # запрос к редактору
```
