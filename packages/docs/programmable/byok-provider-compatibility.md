...
OpenWeave has no backend. When you bring your own API key, the browser talks to the provider
...
| Anthropic            | `https://api.anthropic.com`                       | ✅      | Requires `anthropic-dangerous-direct-browser-access: true`; OpenWeave sends it in the web app | 2026-07-30 |
...
on the **actual response** will fail in the browser with a generic `Failed to fetch`. OpenWeave
...
OpenWeave cannot send this today — see
[#454](https://github.com/openweave/openweave/issues/454).
