...
```tsx
<NumberFieldRoot
  modelValue={layout.node?.width ?? 0}
  onModelValueChange={(value) => layout.updateAxisSize('width', value)}
  onCommit={(value, previous) => layout.commitAxisSize('width', value, previous)}
/>
```
