...
`openweave eval` runs JavaScript against an OpenWeave document with a Figma-compatible `figma` global. Use it for headless batch edits, inspection, fixture setup, and automation without opening the editor UI.

## Basic usage

```sh
openweave eval design.fig -c "return figma.currentPage.children.length"
```

The `-c` flag accepts JavaScript. If the code does not start with `return`, OpenWeave wraps it in an async function and returns the value from that function when present.

```sh
openweave eval design.fig -c "
...
```sh
openweave eval design.fig -c "figma.currentPage.name = 'Updated'" -o updated.fig
```

## Read scripts from stdin

```sh
cat transform.js | openweave eval design.fig --stdin --write
```
...
```sh
openweave eval -c "return figma.currentPage.name"
```
...
```sh
openweave eval design.fig -c "return figma.currentPage.children.map((n) => n.name)" --json
```
...
The API is intentionally close to Figma's Plugin API, but it maps to OpenWeave's scene graph and file format.
...
- `figma.loadFontAsync(fontName)` no-ops because OpenWeave does not gate text edits on plugin font loading
...
Use OpenWeave CLI export commands, core tools, or direct scene-graph helpers where available.
