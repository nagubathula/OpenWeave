...
```sh
openweave export design.fig                           # PNG (default)
openweave export design.fig -f jpg -s 2 -q 90        # JPG at 2×, quality 90
openweave export design.fig -f webp -s 3             # WEBP at 3×
openweave export design.fig -f svg                   # SVG vector
openweave export design.fig -f fig --page "Page 1"   # export one page as .fig
openweave export design.fig -f fig --node 1:23        # export one node as .fig
openweave export design.fig -f html --css tailwind    # export an HTML fragment with Tailwind classes
```
...
```sh
openweave export design.fig -f jsx --style tailwind
```
...
Also supports `--style openweave` for the native JSX format (see [JSX Renderer](../jsx-renderer)).
...
```sh
openweave export design.fig -f html
openweave export design.fig -f html --css tailwind
```
...
```sh
openweave export design.fig -f html --html standalone --css inline
openweave export design.fig -f html --html standalone --css tailwind
openweave export design.fig -f html --html standalone --css tailwind --assets external
```

Standalone Tailwind output is compiled during export; it does not depend on the Tailwind browser runtime. Use `--assets external` to write CSS and extracted image assets next to the HTML file. Use `--fonts assets` with external assets to resolve detected SceneGraph text fonts through OpenWeave's configured web-font providers and emit local `@font-face` files.
...
```sh
openweave export design.fig --thumbnail --width 1920 --height 1080
```
