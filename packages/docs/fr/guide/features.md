...
**Copier-coller avec Figma** — sélectionnez des nœuds dans Figma, <kbd>⌘</kbd><kbd>C</kbd>, passez à OpenWeave, <kbd>⌘</kbd><kbd>V</kbd>. Les remplissages, contours, auto-layout, texte, effets, rayons de coins et réseaux vectoriels sont préservés. Fonctionne dans les deux sens.
...
CLI : `openweave export design.fig -f jsx --style tailwind`
...
```sh
openweave tree design.fig          # Arbre de nœuds
openweave find design.fig --type TEXT  # Recherche
openweave export design.fig -f png     # Rendu
openweave analyze colors design.fig    # Audit des couleurs
openweave analyze clusters design.fig  # Motifs répétés
openweave eval design.fig -c "..."     # API Plugin Figma
```

Lorsque l'application de bureau est lancée, omettez le fichier pour contrôler l'éditeur en direct via RPC :

```sh
openweave tree                     # Document en direct
openweave export -f png            # Capture du canevas
```

Toutes les commandes supportent `--json`. Installation : `npm install -g @openweave/cli`
...
**Web** — disponible sur [app.openweave.dev](https://app.openweave.dev), installable en PWA sur mobile avec interface tactile optimisée.
...
Lorsqu'une police n'est pas disponible localement, OpenWeave la récupère automatiquement depuis Google Fonts. Aucune installation manuelle nécessaire lors de l'ouverture de fichiers .fig avec des polices inconnues.
