...
`openweave eval` vous donne accès à l'API complète Figma Plugin dans le terminal. Lisez des nœuds, modifiez des propriétés, créez des formes — puis écrivez les changements dans le fichier.
...
```sh
cat transform.js | openweave eval design.fig --stdin -w
```
...
```sh
openweave eval -c "figma.currentPage.name"
```
...
```sh
openweave eval design.fig -c "..." --json
```
