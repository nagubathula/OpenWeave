...
**Copiar y pegar con Figma** — selecciona nodos en Figma, <kbd>⌘</kbd><kbd>C</kbd>, cambia a OpenWeave, <kbd>⌘</kbd><kbd>V</kbd>. Rellenos, trazos, auto-layout, texto, efectos, radios de esquina y redes vectoriales se preservan. Funciona en ambas direcciones.
...
```sh
openweave tree design.fig          # Árbol de nodos
openweave find design.fig --type TEXT  # Buscar
openweave export design.fig -f png     # Renderizar
openweave analyze colors design.fig    # Auditoría de colores
openweave analyze clusters design.fig  # Patrones repetidos
...
```sh
openweave tree                     # Documento en vivo
openweave export -f png            # Captura del canvas
```

Todos los comandos soportan `--json`. Instalar: `npm install -g @openweave/cli`
...
**Web** — funciona en [app.openweave.dev](https://app.openweave.dev), instalable como PWA en móvil con interfaz optimizada para táctil.
...
Cuando una fuente no está disponible localmente, OpenWeave la carga automáticamente desde Google Fonts. No requiere instalación manual al abrir archivos .fig con fuentes desconocidas.
