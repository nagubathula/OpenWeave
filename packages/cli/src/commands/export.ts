...
import type { RasterExportFormat } from '@openweave/core/io'
import {
  exportHTMLBundle,
...
const JSX_STYLES = new Set(['openweave', 'tailwind'])
...
export default defineCommand({
  ...
  args: {
    ...
    style: {
      type: 'string',
      description: 'JSX style: openweave or tailwind (default: openweave)',
      default: 'openweave'
    },
    html: {
      type: 'string',
  ...
  async run({ args }) {
    ...
    if (format === 'JSX' && !JSX_STYLES.has(args.style)) {
      printError(`Invalid JSX style "${args.style}". Use openweave or tailwind.`)
      process.exit(1)
    }
