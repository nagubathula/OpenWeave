...
import { normalizeFontFamily } from '@openweave/core/text'
import {
  exportWebFontFaceAssets,
  type WebFontFaceAsset,
  type WebFontFaceRequest
} from '@openweave/core/text/web-font/assets'
...
async function exportStandaloneHTML(
  ...
  if (options.assets === 'external') {
    ...
    body = extracted.html
    const cssPath = `${options.assetBasePath}/openweave.css`
    const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">${stylesheetLink(cssPath)}</head><body><main data-openweave-html="standalone" class="op-stage">${body}</main></body></html>`
    return {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">${styleTag(css)}</head><body><main data-openweave-html="standalone" class="op-stage">${body}</main></body></html>`
