...
describe('@openweave/dom-css runtime', () => {
  ...
  it('serializes inline styles as Tailwind classes when requested', () => {
        ...
        children: [
          {
            ...
            children: [{ type: 'text', text: 'OpenWeave' }]
          }
        ]
      },
      { style: 'tailwind' }
    ...
    expect(html).toBe('<section class="card flex p-4 gap-2 bg-white">OpenWeave</section>')
  })
  ...
  it('exports standalone HTML documents when requested', async () => {
    ...
    expect(html).toContain('data-openweave-html="standalone"')
  ...
  it('uses the headless runtime outside browser contexts', () => {
    ...
    expect(runtime.serializeHTML(cardDocument)).toContain('OpenWeave')
  })

  it('parses HTML with inline styles', () => {
    const runtime = createHeadlessCSSRuntime()
    const document = runtime.parseHTML(
      '<section class="card" style="width: 320px; color: rgb(17, 24, 39)">OpenWeave</section>'
    )
    ...
    expect(section.children[0]).toEqual({ type: 'text', text: 'OpenWeave' })
  })

  it('parses inline style values with embedded semicolons', () => {
    const runtime = createHeadlessCSSRuntime()
    const document = runtime.parseHTML(
      '<section style="background-image: url(\'data:image/svg+xml;utf8,<svg></svg>\'); width: 320px">OpenWeave</section>'
    )
    const section = document.children[0]
  ...
  it('computes selector specificity, inheritance, and shorthands', async () => {
    ...
    const parsed = runtime.parseHTML(`
      <article id="hero" class="card featured">
        <header><h1 class="title">OpenWeave</h1></header>
      </article>
    `)
