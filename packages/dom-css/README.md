...
DOM and CSS projection utilities for OpenWeave.

This package is the compatibility layer between OpenWeave's scene graph and DOM-shaped design documents. It is intentionally separate from `@openweave/core` so browser/CSS parser integrations can evolve without adding DOM dependencies to the renderer and editor core.

## Installation

```sh
bun add @openweave/dom-css @openweave/core
```

`@openweave/core` is a peer dependency because `@openweave/dom-css` projects to and from OpenWeave scene graphs. Consumers that only parse/serialize DesignDOM still need the peer installed for the package entrypoint.
...
```ts
import { createBrowserCSSRuntime } from '@openweave/dom-css'

const runtime = createBrowserCSSRuntime({ sandbox: 'iframe' })
const document = runtime.parseHTML('<article class="card">OpenWeave</article>')
...
```ts
import { createHeadlessCSSRuntime } from '@openweave/dom-css'
...
```ts
import { htmlToSceneGraph } from '@openweave/dom-css'

const graph = await htmlToSceneGraph(
  '<article class="card"><h1>OpenWeave</h1></article>',
  {
...
```tsx
/** @jsxImportSource @openweave/dom-css */
import { createBrowserCSSRuntime, jsxToSceneGraph } from '@openweave/dom-css'
...
When running in a browser, use the browser-first helpers so native `getComputedStyle()` is used automatically. Import them from `@openweave/dom-css/browser` so browser bundles do not load headless-only CSSOM dependencies:

```tsx
/** @jsxImportSource @openweave/dom-css */
import { browserJSXToSceneGraph } from '@openweave/dom-css/browser'
...
```ts
import { browserHTMLToSceneGraph } from '@openweave/dom-css/browser'

import tailwindCSS from './generated-tailwind.css?raw'

const graph = await browserHTMLToSceneGraph(
  '<article class="flex w-80 rounded-xl bg-white p-6">OpenWeave</article>',
  {
    cssText: tailwindCSS,
    sandbox: 'iframe'
  }
...
```ts
import { browserTailwindHTMLToSceneGraph } from '@openweave/dom-css/browser'

const classes = ['flex', 'w-80', 'rounded-xl', 'bg-white', 'p-6']
const graph = await browserTailwindHTMLToSceneGraph(
  `<article class="${classes.join(' ')}">OpenWeave</article>`,
  classes,
  {
...
```ts
import { tailwindHTMLToSceneGraph } from '@openweave/dom-css'
...
- Map more computed CSS properties to scene graph fields through browser-native computed style or dependency-backed parsers: richer shadows, typography details, position constraints, borders, gradients, and grid once OpenWeave's grid support matures
- Improve SceneGraph → CSS export so generated HTML/CSS is useful for JSX, Tailwind, and web export
- Keep `@openweave/dom-css` stable before splitting lower-level file-format packages such as future `@openweave/kiwi` and `@openweave/fig`
