...
import { copyFill, copyStroke } from '@openweave/scene-graph/copy'
...
export function setNodeFillOkHCL(
  ...
  return {
    fills,
    pluginData: payloads.map((value) => ({ pluginId: 'openweave', key: OKHCL_PLUGIN_KEY, value }))
  }
}
...
export function getNodeOkHCLPayloads(node: SceneNode): OkHCLPayload[] {
  return node.pluginData
    .filter((entry) => entry.pluginId === 'openweave' && entry.key === OKHCL_PLUGIN_KEY)
