...
import type { Color, GUID, Matrix, Vector } from '@openweave/scene-graph/primitives'
...
function materializeFigmaBlob(
  value: { __openWeaveFigmaBlob?: Uint8Array | Record<string, number> },
  blobs: Uint8Array[],
  options: MaterializeFigmaPayloadOptions
): number {
  const blob = value.__openWeaveFigmaBlob
...
function materializeFigmaPayload(
  ...
  if ('__openWeaveFigmaBlob' in value) {
    return materializeFigmaBlob(
      value as { __openWeaveFigmaBlob?: Uint8Array | Record<string, number> },
      blobs,
      options
    )
  }
