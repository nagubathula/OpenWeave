import type { NodeChange, PluginData, PluginRelaunchData } from '@openweave/kiwi/fig/codec'
...
export const OPEN_PENCIL_PLUGIN_ID = 'openweave'
...
export function applyExportSettingsPluginData(
  ...
  if (
    !hasOpenWeaveExportSettingsPluginData(node.pluginData) &&
...
function hasOpenWeaveExportSettingsPluginData(pluginData: PluginDataEntry[]): boolean {
...
export function extractBoundVariables(nc: NodeChange): Record<string, string> {
  const bindings = parseBoundVariablesPluginValue(
    getOpenWeavePluginValue(nc, BOUND_VARIABLES_PLUGIN_KEY)
  )
...
export function extractExportSettings(nc: NodeChange): ExportSetting[] {
  const pluginSettings = parseExportSettingsPluginValue(
    getOpenWeavePluginValue(nc, EXPORT_SETTINGS_PLUGIN_KEY)
  )
...
export function getOpenWeavePluginValue(nc: NodeChange, key: string): string | null {
  return (
