...
export { guidToString, stringToGuid } from '@openweave/kiwi/fig/guid'
...
function convertTextProps(nc: NodeChange, blobs: Uint8Array[]): TextProps {
  return {
    ...
    textDirection:
      (getOpenWeavePluginValue(nc, TEXT_DIRECTION_PLUGIN_KEY) as
...
> &
  ...
  return {
    ...
    layoutDirection:
      (getOpenWeavePluginValue(nc, LAYOUT_DIRECTION_PLUGIN_KEY) as
...
function resolveNodeType(nc: NodeChange): NodeType | 'DOCUMENT' | 'VARIABLE' {
  ...
  if (
    ...
    getOpenWeavePluginValue(nc, NODE_TYPE_PLUGIN_KEY) === 'COMPONENT_SET'
  ) {
...
interface PreservedFigmaBlob {
  __openWeaveFigmaBlob: Uint8Array
}
