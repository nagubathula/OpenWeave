 ...
 *
 * Socket directory layout (overridable via OPENWEAVE_MCP_SOCKET):
 *   macOS:   ~/Library/Application Support/OpenWeave/
 *   Linux:   $XDG_RUNTIME_DIR/openweave/  (fallback: ~/.openweave/)
 *   Windows: %LOCALAPPDATA%\OpenWeave\  (fallback: ~\AppData\Local\OpenWeave\)
 *
 ...
 * OPENWEAVE_MCP_DISCOVERY_PATH is set (see getDiscoveryPath()). The socket
 * override (OPENWEAVE_MCP_SOCKET) never moves the discovery file — it is
 ...
 * OPENWEAVE_MCP_SOCKET is set, the discovery file stays at getPlatformDir().
 */

const DIR_NAME_UNIX = 'openweave'
const DIR_NAME_MACOS = 'OpenWeave'
 ...
 * ignoring OPENWEAVE_MCP_SOCKET. The discovery file always lives here so
 ...
 *
 * When OPENWEAVE_MCP_SOCKET is set, its dirname is used as the socket
 ...
 * OPENWEAVE_MCP_SOCKET. This function should NOT be used to locate it.
 */
export async function getSocketDir(): Promise<string> {
  const socketOverride = process.env.OPENWEAVE_MCP_SOCKET?.trim()
 ...
 *
 * When OPENWEAVE_MCP_SOCKET is set, its value is returned directly
 ...
 * find it without knowing whether OPENWEAVE_MCP_SOCKET is set. It contains
 ...
 * way around.
 *
 * Set OPENWEAVE_MCP_DISCOVERY_PATH to relocate the discovery file (e.g. to a
...
export async function getDiscoveryPath(): Promise<string> {
  const override = process.env.OPENWEAVE_MCP_DISCOVERY_PATH?.trim()
  if (override) {
