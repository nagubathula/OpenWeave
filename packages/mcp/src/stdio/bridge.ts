...
const DISCONNECTED_MESSAGE =
  'OpenWeave app is not connected. ' +
  'STOP and tell the user: "The OpenWeave desktop app is not running or no document is open. ' +
...
export function createStdioRpcBridge({
  ...
  //   Hint:     OPENWEAVE_MCP_SOCKET env var — resolved through
  ...
  function sendRpc(body: Record<string, unknown>): Promise<unknown> {
    ...
    const awaitReady = async (): Promise<void> => {
      ...
      if (authFailure) throw new Error('Unauthorized: check OPENWEAVE_MCP_AUTH_TOKEN')
            ...
            httpRequest('POST', '/rpc', body)
              .then(({ status, data, req }) => {
                ...
                if (status === 401) {
                  ...
                  settled = true
                  scheduleReconnect()
                  reject(new Error('Unauthorized: check OPENWEAVE_MCP_AUTH_TOKEN'))
                  return undefined
                }
