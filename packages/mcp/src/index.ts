...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  process.stdout.write(
    `openweave-mcp-http\n\n` +
      `Start the OpenWeave MCP server.\n\n` +
      ...
      `  OPENWEAVE_MCP_SOCKET        Override Unix socket path (recorded in the discovery file)\n` +
      `  OPENWEAVE_MCP_DISCOVERY_PATH Override discovery file (mcp.json) location; defaults to the\n` +
      `                               platform path. Parent dir created 0o700. Mainly for test isolation.\n` +
      `  OPENWEAVE_MCP_TCP           Deprecated — TCP is controlled by PORT (>0 = on, 0 = off)\n` +
      `  OPENWEAVE_MCP_AUTH_TOKEN    Bearer token for MCP and RPC auth\n` +
      `  OPENWEAVE_MCP_ROOT          Allowed directory for file-scoped tools (default: current working directory)\n` +
      `  OPENWEAVE_MCP_EVAL          Set to 1 to enable the eval tool\n` +
      `  OPENWEAVE_MCP_CORS_ORIGIN   Allowed CORS origin\n`
  )
  process.exit(0)
}
...
const port = rawPort
// OPENWEAVE_MCP_TCP is accepted for backward compat but has no effect —
...
const handle = await startServer({
  ...
  withTcp,
  socketPath: process.env.OPENWEAVE_MCP_SOCKET?.trim() || null,
  enableEval: process.env.OPENWEAVE_MCP_EVAL === '1',
  mcpRoot: process.env.OPENWEAVE_MCP_ROOT?.trim() || process.cwd(),
  ...
  // set an explicit one.
  authToken: (() => {
    const raw = process.env.OPENWEAVE_MCP_AUTH_TOKEN
    if (raw === undefined) return undefined
    if (raw === '') return null
    const trimmed = raw.trim()
    if (!trimmed) {
      process.stderr.write(
        'Error: OPENWEAVE_MCP_AUTH_TOKEN is whitespace-only. Set a real token, or use an empty string to disable auth.\n'
      )
      process.exit(1)
    }
    return trimmed
  })(),
  corsOrigin: process.env.OPENWEAVE_MCP_CORS_ORIGIN?.trim() || null
})

process.stderr.write(`OpenWeave MCP server\n`)
