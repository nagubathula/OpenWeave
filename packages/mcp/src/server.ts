...
async function resolveMcpInstallCommand(): Promise<string> {
  const agent =
    ...
    'npm'
  const resolved = resolveCommand(agent, 'global', [`@openweave/mcp@${MCP_VERSION}`])
  if (!resolved) return `npm install -g @openweave/mcp@${MCP_VERSION}`
...
function buildServerContext(options: ServerOptions) {
  ...
  // Auto-generated so all transports require auth by default. Override via OPENWEAVE_MCP_AUTH_TOKEN or authToken option.
  ...
  if (authToken === null && withTcp) {
    process.stderr.write(
      `WARNING: MCP server is running without authentication on TCP port ${httpPort}. ` +
        ...
        'Set OPENWEAVE_MCP_AUTH_TOKEN to enable auth, or use PORT=0 for socket-only transport.\n'
    )
  }
