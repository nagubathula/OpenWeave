...
import { initCanvasKit } from '@openweave/core/io'
...
const { values: opts } = parseArgs({
  options: {
    figma: {
      type: 'string',
      default: '/tmp/openweave-oracles/pattern-visible-source-tuned/figma.png'
    },
    ours: {
      type: 'string',
      default: '/tmp/openweave-oracles/pattern-visible-source-tuned/ours.png'
    },
...
if (!existsSync(oursPath)) throw new Error(`Missing OpenWeave image: ${oursPath}`)
...
function rowDeltaSummary(
  ...
  return {
    ...
    missingOpenWeaveRows: Math.max(0, figmaRowCount - openWeaveRowCount),
    extraOpenWeaveRows: Math.max(0, openWeaveRowCount - figmaRowCount)
  }
}
