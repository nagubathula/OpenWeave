...
import { headlessRenderNodes, initCanvasKit, parseFigFile } from '@openweave/core/io'
import { computeAllLayouts } from '@openweave/core/layout'
import type { SceneGraph } from '@openweave/core/scene-graph'
...
const { values, positionals } = parseArgs({
  ...
  options: {
    ...
    output: { type: 'string', short: 'o', default: '/tmp/openweave-visual-bisect' },
...
Options:
  --output DIR          Output directory (default: /tmp/openweave-visual-bisect)
  ...
  --root-node-id ID     OpenWeave node whose children should be bisected
...
What it does:
  ...
  and OpenWeave, exports that subset from both renderers, diffs the images,
...
const outputDir = values.output ?? '/tmp/openweave-visual-bisect'
...
async function compareSubset(indices: number[], depth: number): Promise<BisectResult> {
  ...
  const openWeavePath = `${outputDir}/${stem}-openweave.png`

  await exportFigmaSubset(figmaRootId, indices, figmaPath)
  await exportOpenWeaveSubset(indices, openWeavePath)

  return {
    depth,
    indices,
    names: indices.map((index) => childNames[index]),
    metrics: await diffImages(figmaPath, openWeavePath),
    figmaPath,
    openWeavePath
  }
}

async function exportOpenWeaveSubset(indices: number[], path: string): Promise<void> {
  ...
  try {
    ...
    if (!data) throw new Error(`OpenWeave render produced no image for ${indices.join(',')}`)
...
async function writeReport(items: BisectResult[]): Promise<void> {
  ...
  for (const item of items) {
    lines.push(
      `| ${item.depth} | ${item.indices.join(',')} | ${item.metrics.above40.toFixed(3)}% | ${item.metrics.above20.toFixed(3)}% | ${item.metrics.mean.toFixed(3)} | ${item.names.map(escapePipes).join('<br>')} | [figma](${item.figmaPath}) / [openweave](${item.openWeavePath}) |`
    )
  }
