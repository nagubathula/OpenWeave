...
const { values } = parseArgs({
  options: {
    output: { type: 'string', short: 'o', default: '/tmp/openweave-fixture-visuals' },
    heavy: { type: 'boolean', default: false }
  }
})

const outputDir = values.output ?? '/tmp/openweave-fixture-visuals'
...
for (const target of TARGETS) {
  ...
  await $`bun openweave export ${target.file} --page ${target.page} --output ${outputPath}`
}
