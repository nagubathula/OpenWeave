#!/usr/bin/env bun
/**
 * Visual comparison pipeline: Figma vs OpenWeave renderer.
 *
 ...
 *   ours.png   — rendered by OpenWeave headless SkiaRenderer
...
import { importClipboardNodes, parseFigmaClipboard } from '@openweave/core/clipboard'
import { renderNodesToImage, initCanvasKit } from '@openweave/core/io'
import { computeAllLayouts } from '@openweave/core/layout'
import { SceneGraph } from '@openweave/core/scene-graph'
...
async function runWithClipboard() {
  ...
  console.log('🖼️  Rendering with OpenWeave…')
...
async function diff() {
  ...
  const metrics = {
    figmaSize,
    openWeaveSize: oursSize,
    comparedOpenWeavePath: compareOursPath,
