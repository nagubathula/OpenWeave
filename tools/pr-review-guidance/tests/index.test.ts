...
async function writeEvent(event: unknown) {
  const dir = await mkdtemp(join(tmpdir(), 'openweave-pr-guidance-'))
  ...
  test('logs a maintainer note and only reads PR data', async () => {
    ...
    await monitorPRReviewGuidance({
      env: {
        ...
        GITHUB_REPOSITORY: 'openweave/openweave',
        GITHUB_REPOSITORY_OWNER: 'openweave',
        GITHUB_TOKEN: 'token'
      },
    ...
    expect(requests).toEqual(['GET https://example.test/repos/openweave/openweave/pulls/294'])
