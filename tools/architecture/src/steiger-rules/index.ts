...
const preferDomainFoldersOverFilenamePrefixes: Rule = {
  name: 'openweave/prefer-domain-folders-over-filename-prefixes',
  check(root) {
...
const scriptsAreEntrypointShims = createTextRule(
  'openweave/scripts-are-entrypoint-shims',
  (sourceRel, content) => {
...
const strictToolsLayout = createFileRule('openweave/strict-tools-layout', (sourceRel) => {
...
const strictTestFilePlacement = createFileRule(
  'openweave/strict-test-file-placement',
  (sourceRel) => {
...
const noMisplacedEngineTestDomainPaths = createFileRule(
  'openweave/no-misplaced-engine-test-domain-paths',
  (sourceRel) => {
...
const noKitchenSinkEngineBasicTests: Rule = {
  name: 'openweave/no-kitchen-sink-engine-basic-tests',
  check(root) {
    const diagnostics: Diagnostic[] = []
...
const noEngineOnlyAssertionsInE2E = createImportRule(
  'openweave/no-engine-only-assertions-in-e2e',
...
const noE2EImportsInEngineTests = createImportRule(
  'openweave/no-e2e-imports-in-engine-tests',
...
const noAppImportsInWorkspacePackages = createImportRule(
  'openweave/no-app-imports-in-workspace-packages',
  (sourceRel, specifier, resolved) => {
...
const noPackageInternalsInApp = createImportRule(
  'openweave/no-package-internals-in-app',
  (sourceRel, specifier, resolved) => {
...
const noForeignPackageLocalAliases = createImportRule(
  'openweave/no-foreign-package-local-aliases',
  (sourceRel, specifier) => {
...
const noComponentsImportViews = createImportRule(
  'openweave/no-components-import-views',
  (sourceRel, _specifier, resolved) => {
...
const noNonUiImportsInSharedUi = createImportRule(
  'openweave/no-non-ui-imports-in-shared-ui',
  (sourceRel, _specifier, resolved) => {
...
const noAppImportsInSharedUi = createImportRule(
  'openweave/no-app-imports-in-shared-ui',
  (sourceRel, _specifier, resolved) => {
    if (!sourceRel.startsWith('src/components/ui/')) return null
...
const noPropertyPanelInternalsOutsidePanel = createImportRule(
  'openweave/no-property-panel-internals-outside-panel',
  (sourceRel, _specifier, resolved) => {
...
const noProductionTestIdsInSharedLayers = createTextRule(
  'openweave/no-production-test-ids-in-shared-layers',
  (sourceRel, content) => {
    const inSharedLayer =
...
const noUiImportsInCore = createImportRule(
  'openweave/no-ui-imports-in-core',
  (sourceRel, specifier) => {
...
export const openWeaveArchitecturePlugin = {
  meta: { name: 'openweave-architecture', version: '0.0.0' },
  ruleDefinitions: [
