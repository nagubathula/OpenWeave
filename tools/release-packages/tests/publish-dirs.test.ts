...
async function fixtureRoot() {
  const root = join(tmpdir(), `openweave-release-packages-${crypto.randomUUID()}`)
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        name: '@openweave/example',
        version: '1.0.0',
        scripts: { build: 'tsdown' },
        dependencies: { '@openweave/core': 'workspace:*', zod: '^4.0.0' },
...
describe('publishPackageJSON', () => {
  test('rewrites workspace dependencies and strips private build fields', () => {
    ...
    expect(json).toEqual({
      name: '@openweave/example',
      dependencies: { '@openweave/core': '^0.13.2', zod: '^4.0.0' },
      main: './dist/index.js'
    })
  })
})
