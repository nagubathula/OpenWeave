...
import { JSX_REFERENCE, selectionToJSX } from '@openweave/core/design-jsx'
import { useI18n, useSceneComputed } from '@openweave/vue'
...
import type { JSXFormat } from '@openweave/core/design-jsx'

const store = useEditorStore()
const { copy, copied } = useClipboard({ copiedDuring: 2000 })
const { dialogs } = useI18n()
const jsxFormat = ref<JSXFormat>('openweave')
...
function toggleFormat() {
  jsxFormat.value = jsxFormat.value === 'openweave' ? 'tailwind' : 'openweave'
}
    ...
    >
      <div class="flex items-center gap-1.5">
        ...
        <AppTextButton
          ...
          @click="toggleFormat"
        >
          {{ jsxFormat === 'openweave' ? 'OpenWeave' : 'Tailwind' }}
        </AppTextButton>
      </div>
