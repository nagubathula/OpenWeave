...
export interface AppSelectProps<TValue extends string | number> extends Omit<SelectPrimitive.SelectProps, 'value' | 'onValueChange'> {
  label?: string
    ...
    return (
      <SelectPrimitive.Root value={stringValue} onValueChange={handleValueChange} {...props}>
        <SelectPrimitive.Trigger
          ...
          aria-label={label}
        >
          <SelectPrimitive.Value placeholder={placeholder} className={styles.value({ className: ui?.value })} />
          ...
          >
            ...
            <SelectPrimitive.Viewport className={styles.viewport({ className: ui?.viewport })}>
              {options.map((opt) => (
                ...
                >
                  <SelectPrimitive.ItemIndicator className={styles.indicator({ className: ui?.indicator })}>
