import { encodeBase64 } from '@openweave/core/bytes'
import { selectionToJSX, sceneNodeToJSX, type RasterExportFormat } from '@openweave/core/io'
...
export async function handleExportJsx(target: AutomationTarget, args: unknown): Promise<unknown> {
  ...
  const style = (jsxArgs?.style ?? 'openweave') as 'openweave' | 'tailwind'
