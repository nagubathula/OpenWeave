...
import type { Editor, EditorState } from '@openweave/core/editor'
import type {
  ExportRequest,
  IOFormatAdapter,
  IORegistry,
  RasterExportFormat
} from '@openweave/core/io'
import { renderNodesToImage } from '@openweave/core/io/formats/raster'
...
export function getExportOptions(formatId: string, options?: ExportOptions): unknown {
  ...
  if (formatId === 'jsx') return { format: options?.jsxFormat ?? 'openweave' }
  return undefined
}
