...
import { randomIndex } from '@openweave/core/random'
...
export function createFollowActions(
  ...
  function tickFollow() {
    ...
    const followedClientId = followingPeer.get()
    if (!followedClientId || !awareness) return
    const peerState = awareness.getStates().get(followedClientId)
    if (!peerState?.cursor) {
      followingPeer.set(null)
      return
    }
